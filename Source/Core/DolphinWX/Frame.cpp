...
EVT_MENU(IDM_RUN_SCRIPT, CFrame::OnScriptRun)
EVT_MENU(IDM_CANCEL_SCRIPT, CFrame::OnScriptTerminate)
...
CFrame::CFrame(wxFrame *parent, wxWindowID id, const wxString &title, const wxPoint &pos, const wxSize &size,
               bool _UseDebugger, bool _BatchMode, bool ShowLogWindow, long style)
    : CRenderFrame(parent, id, title, pos, size, style)
    , g_pCodeWindow(nullptr)
    , g_NetPlaySetupDiag(nullptr)
    , g_CheatsWindow(nullptr)
    , m_SavedPerspectives(nullptr)
    , m_ToolBar(nullptr)
    , m_GameListCtrl(nullptr)
    , m_Panel(nullptr)
    , m_RenderFrame(nullptr)
    , m_RenderParent(nullptr)
    , m_LogWindow(nullptr)
    , m_LogConfigWindow(nullptr)
    , m_FifoPlayerDlg(nullptr)
    ...
    , m_menubar_shadow(nullptr)
{
	...
	Movie::SetWiiInputManip(
	    [this](u8 *data, WiimoteEmu::ReportFeatures rptf, int controllerID, int ext, const wiimote_key key)
...
{
	if (!Core::IsRunning() || !SConfig::GetInstance().bRenderWindowAutoSize || RendererIsFullscreen() ||
	...
	if (SConfig::GetInstance().bRenderToMain &&
	    (SConfig::GetInstance().m_InterfaceLogWindow || SConfig::GetInstance().m_InterfaceLogConfigWindow) &&
...
void CFrame::ParseHotkeys()
{
	...
	if (IsHotkey(HK_RUN_SCRIPT))
	{
		wxCommandEvent scriptEvent;
		CFrame::OnScriptRun(scriptEvent);
	}

	if (IsHotkey(HK_CANCEL_SCRIPT))
	{
		wxCommandEvent scriptEvent;
		CFrame::OnScriptTerminate(scriptEvent);
	}
}
